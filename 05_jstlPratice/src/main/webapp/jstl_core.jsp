...
<body>
	...
	<%-- TODO: result 변수를 session 영역에 (num1과 num2의 합을 저장) --%>
	...
	<%--
	  	* c:out
	  		- EL 표현식으로 값을 출력
	  		- default 속성 : 값이 null이거나 없을 때 기본값으로 출력
	   --%>
	result (c:out) :
	<c:out value="${ result }" />
	<br> result (c:out + default) :
	...
	color.equals == "red" (x) --> color.equals("red")
	<c:if test="${ color == 'red' }">
		<p style: "${color }">$[color]
		</p>
	</c:if>
	<%-- Java에서는 문자열 비교 시 .equals()를 사용해야 비교가 되었는데,
	   		EL에서는 == 연산자로 문자열 비교가 가능!
	   	 --%>
	...
	<%--
	   	 	...
	   	 	* c:when
	   	 		- 조건을 작성하며, 각 조건이 true 인 블록만 실행 (처음으로 true 인 블록이 있으면 해당 블록만 실행)
	   	 	* c:otherwise
	   	 		-	모든 c:when 이 false 일 때 실행되는 기본값 블록( else, default과 유사)
	   	  --%>
	<c:choose>
		<c:when test="${ n1 > 30 }">
			<p>n1이 30보다 큽니다.</p>
	...
	<%
		...
		list.add(new MemberDTO(1,"권경환","kh__@gmail.com",39));
		list.add(new MemberDTO(2,"김상명","sm__@gmail.com",20));
		list.add(new MemberDTO(3,"장현명","hm__@gmail.com",30));
	%>
